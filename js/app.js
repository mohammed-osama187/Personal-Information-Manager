...
function fireWebNotification(title, bodyText) {
...
function initAuth() {
    ...
    window.closeAuthScreen = function(event) {
        ...
        authScreen.style.display = 'none';
        appScreen.style.display = 'flex';
        // Immediately fetch guest tasks on entering guest mode
        displayTasks();
        if(window.calendarInstance) window.calendarInstance.refetchEvents();
        initSettings();
    };
    ...
    if (window.auth) {
        window.onAuthStateChanged(window.auth, (user) => {
            if (user) {
                // المستخدم مسجل دخول
                const currentUser = {
                    id: user.uid, // استخدام الـ UID الموحد من فايربيز
                    email: user.email,
                    name: user.displayName || user.email.split('@')[0]
                };
                localStorage.setItem('currentUser', JSON.stringify(currentUser));
                localStorage.removeItem('isGuestMode'); // No longer in guest mode
                ...
                if (justLoggedOut === 'true') {
                    ...
                    // Stay on the sign-up page (which already has continue as guest)
                    authScreen.style.display = 'flex';
                    appScreen.style.display = 'none';
                }
    ...
    getTasksFromFirebase().then(allItems => {
        ...
        const createHTML = (item, isHistory = false) => {
            ...
            if (item.type === 'habit' && !isHistory) {
                ...
                const firstDayIndex = new Date(curYear, curMonth, 1).getDay(); // 0 (Sun) to 6 (Sat)
...
window.toggleItemComplete = function(id, currentStatus) {
    getTasksFromFirebase().then(tasks => {
        ...
        if (item.type === 'habit') {
            ...
            if (idx > -1) {
                // Already checked today, uncheck it
                item.completedDates.splice(idx, 1);
                item.isCompleted = false;
                item.completedCount = 0;
            } else {
                // Not checked today, check it
...
window.toggleHabitDay = function(event, id, dateStr) {
    if (event) event.stopPropagation();
    getTasksFromFirebase().then(tasks => {
        const item = tasks.find(t => String(t.id) === String(id));
        if (!item) return;
        ...
        // Update main isCompleted / completedCount status based on today's completion status
        const now = new Date();
        const year = now.getFullYear();
        const month = String(now.getMonth() + 1).padStart(2, '0');
        const day = String(now.getDate()).padStart(2, '0');
...
function initPomodoroDrag() {
    ...
    function updateTimeDisplay() {
        ...
        if (clockText.textContent !== newText) {
