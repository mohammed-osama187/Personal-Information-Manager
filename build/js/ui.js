...
export function initSettings() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        import('./firebase.js').then(module => {
            ...
            module.updateProfile(user, { displayName: newName })
                .then(() => {
    ...
    if (passForm) {
        passForm.addEventListener('submit', (e) => {
            ...
            import('./firebase.js').then(module => {
                ...
                const credential = module.EmailAuthProvider.credential(user.email, currentPass);
                module.reauthenticateWithCredential(user, credential)
                    .then(() => {
                        return module.updatePassword(user, newPass);
                    })
                    .then(() => {
                        passForm.reset();
        ...
        getTasksFromFirebase().then(allItems => {
            ...
            if (!currentUser) {
                ...
                saveTaskToFirebase(dataItem).then(() => {
                    if (dataItem.id && scheduleItemNotifications) {
                        scheduleItemNotifications(dataItem);
                    }
            ...
            import('./firebase.js').then(module => {
                ...
                if (editId) {
                    const docRef = module.doc(module.db, "tasks", String(editId));
                    ...
                    module.updateDoc(docRef, updateData).then(() => {
                        dataItem.id = editId;
                    ...
                    module.addDoc(module.collection(module.db, "tasks"), dataItem).then((docRef) => {
        ...
        const createHTML = (item, isHistory = false) => {
            ...
            if (item.type === 'habit' && !isHistory) {
                ...
                for (let s = 0; s < firstDayIndex; s++) {
                    gridHTML += `<div class="day-circle empty"></div>`;
                }
                
                for (let d = 1; d <= daysInMonth; d++) {
                    const dayDateStr = `${curYear}-${String(curMonth + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                    
                    let dayClass = 'day-circle';
                    let dayContent = `${d}`;
                    
                    const isBeforeCreation = (curYear < cYear) || 
                                             (curYear === cYear && curMonth < cMonth) || 
                                             (curYear === cYear && curMonth === cMonth && d < cDay);
...
export function sortAndRenderDashboard() {
    ...
    const createHTML = (item) => {
        ...
        const itemClass = item.isCancelled ? 'cancelled' : (item.isCompleted ? 'completed' : '');
        let dueInfo = item.dueDate ? ` <span style="font-size:11px; color:#FF4D4F; margin-left: 8px; font-weight: 500;"><i class="fa-regular fa-calendar-xmark"></i> Due: ${item.dueDate}${item.dueTime ? ' ' + formatTaskTimeDisplay(item.dueTime) : ''}</span>` : '';
        let checkboxHTML = `<input type="checkbox" class="custom-checkbox" ${item.isCompleted ? 'checked' : ''} ${item.isCancelled ? 'disabled' : ''} onchange="toggleItemComplete('${item.id}', ${item.isCompleted})">`;
